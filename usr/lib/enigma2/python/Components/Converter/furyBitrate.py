...
class furyBitrate(Converter, object):
    def __init__(self, type):
        ...
        if path.exists(BITRATE_PATH):
            self.my_console.ePopen('chmod 755 {}'.format(BITRATE_PATH))
    ...
    def getText(self):
        ...
        # Filter out invalid bitrate values (expected unit: Kb/s)
        vcur = self.vcur if 0 <= self.vcur < 1000000 else 0

        # Convert Kb/s (kilobit/s) -> mb/s (megabit/s)
        # 1 mb/s = 1024 Kb/s
        v_mbps = float(vcur) / 1024.0

        # Video only (no 'V:' prefix, no audio) — always show a value,
        # so something appears immediately when the channel starts.
        return "{:.2f} mb/s".format(v_mbps)
    ...
    def run_bitrate(self):
        ...
        try:
            ...
            if stream:
                if DBG:
                    AGDEBUG("[furyBitrate:run_bitrate] Collecting stream data...")
        ...
        if isImageType('vti'):
            cmd = 'killall -9 bitrate > /dev/null 2>&1; nice bitrate {} {} {}'.format(demux, vpid, apid)
        else:
            cmd = 'killall -9 bitrate > /dev/null 2>&1; nice bitrate {} {} {} {}'.format(adapter, demux, vpid, apid)

        if DBG:
            AGDEBUG('[furyBitrate:run_bitrate] starting "{}"'.format(cmd))
    ...
    def app_closed(self, retval):
        if DBG:
            AGDEBUG("[furyBitrate:app_closed] >>> retval={}, is_suspended={}".format(retval, self.is_suspended))
    ...
    def data_avail(self, data):
        if DBG:
            AGDEBUG("[furyBitrate:data_avail] >>> data '{}'\n\tself.remaining_data='{}'".format(data, self.remaining_data))
        ...
        if len(self.data_lines) >= 2:
            try:
                ...
                if DBG:
                    AGDEBUG("[furyBitrate:data_avail] Parse error: {}".format(e))
