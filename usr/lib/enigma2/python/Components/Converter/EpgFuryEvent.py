...
class EpgFuryEvent(Converter, object):
    ...
    def getText(self):
        ...
        # Per-field toggles:
        # - Name in InfoBar controlled by config.plugins.aifury.infobar_translate
        # - Descriptions controlled by config.plugins.aifury.enable_translate_current_event
        allow_name = _is_infobar_name_translate_enabled()
        allow_desc = _is_description_translate_enabled()

        # If nothing is allowed to be translated, return originals immediately
        ...
        # Respect per-field toggles even if cache already contains translations
        if not allow_name:
            name_t = name
        if not allow_desc:
            desc_t = desc_text
        ...
        if ctrl is not None and hasattr(ctrl, "translate_async"):
            try:
                if allow_name and name and name_t is None and name_key and name_key not in _FURY_REQUESTED:
                    _FURY_REQUESTED.add(name_key)
                    ctrl.translate_async(name)
                    need_poll = True
                if allow_desc and desc_text and desc_t is None and desc_key and desc_key not in _FURY_REQUESTED:
