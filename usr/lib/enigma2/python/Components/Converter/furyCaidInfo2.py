...
#  ver 1.3.3 28.11.2025

#  Coded by bigroma & 2boom & j00zek
#  Modified by Islam Salama by adding the server power feature and adding CW types.
...
class furyCaidInfo2(Poll, Converter, object):
    CAID = 0
    PID = 1
    PROV = 2
    ALL = 3
    ...
    # NEW: CW types (capture control words lines that start with cw)
    CW = 49
    CW_C = 50
    ...
    def __init__(self, type):
        ...
        # NEW: request CW display
        elif type == "CW": self.type = self.CW
        elif type == "CwEcm": self.type = self.CW_C
        ...
        if not service:
            ...
            if not info: return softCamName
                ...
                if self.type == self.PROV:
                    return prov
                # ecm time (نستخدمه كمؤشر على قوة / سرعة السيرفر)
                raw_ecm_time = ecm_info.get("ecm time", "")

                if raw_ecm_time.find("msec") > -1:
                    # مثال: "345 msec" أو "345 ms"
                    ecm_time = raw_ecm_time
                    time_digits = raw_ecm_time.replace("msec", "").replace("ms", "").strip()
                else:
                    # مثال: "0.345" ثانية = 345 msec
                    time_digits = raw_ecm_time.replace(".", "").lstrip("0")
                    ecm_time = time_digits + " msec"

                # القيمة الافتراضية للعرض (بدون لون)
                ecm_time_display = ecm_time

                # نحاول نحول القيمة لرقم صحيح عشان نقيم السرعة
                try:
                    ecm_val = int(time_digits)
                except Exception:
                    ecm_val = None

                # لو عرفنا نقرأ القيمة نحدد اللون (أخضر / أصفر / أحمر)
                if ecm_val is not None:
                    # حدود تقديرية جديدة:
                    # 0 - 300 ms    => سيرفر قوي (أخضر)
                    # 301 - 800 ms  => سيرفر متوسط (أصفر)
                    # > 800 ms      => سيرفر ضعيف (أحمر)
                    if ecm_val <= 300:
                        color = r"\c0000FF00"  # أخضر
                    elif ecm_val <= 800:
                        color = r"\c00FFFF00"  # أصفر
                    else:
                        color = r"\c00FF0000"  # أحمر

                    ecm_time_display = "%s%s" % (color, ecm_time)

                # نوع Delay يعرض فقط الـ ECM Time الملون
                if self.type == self.DELAY:
                    return ecm_time_display
                # protocol
                ...
                # NEW: show ONLY cw0 (if present) collected from ecm_info
                if self.type == self.CW or self.type == self.CW_C:
                    # prefer exact 'cw0' key (case-insensitive)
                    cw0 = None
                    for k in ecm_info.keys():
                        if k.lower() == 'cw0':
                            cw0 = ecm_info.get(k, "").strip()
                            keyname = k
                            break
                    if not cw0:
                        return _("no cw0 data")
                    return "%s: %s" % (keyname, cw0)
                ...
                if self.type == self.FORMAT or self.type == self.USE_CFG:
                    ...
                    for param in params:
                        if param != '':
                            ...
                            # ECM Time
                            elif param == "%T": textvalue += ecm_time_display
                            # SoftCamName
                    ...
                    if x != -1:
                        ...
                        if len(item) > 1:
                            ...
                            if item[0] == "Provider":
                                ...
                                item[1] = item[1].strip("\n")
                            elif item[0][:2].lower() == 'cw':
                                # store cw lines like cw0, cw1, etc. (lowercased keys)
                                keyname = item[0].strip().lower()
                                info[keyname] = item[1].strip()
                            elif item[0] == 'ChID' or item[0] == "Service":
                                # leave ChID/Service untouched (avoid storing heavy binary data)
                                pass
