...
# ==============================================
# ServiceInfoEX – Supported Converter Types (examples)
# Use: <convert type="ServiceInfoEX">type_name</convert>
# ==============================================

# PID & IDs
# apid              → Audio PID (hex) e.g. "00A2"
# vpid              → Video PID (hex) e.g. "015A"
# sid               → Service ID (hex) e.g. "6DCA"
# onid              → Original Network ID (hex) e.g. "0044"
# tsid              → Transport Stream ID (hex) e.g. "0001"
# prcpid            → PCR PID (hex) e.g. "01AB"
# pmtpid            → PMT PID (decimal/string) e.g. "200"
# txtpid            → Teletext PID e.g. "1F00"
# caids             → Conditional Access IDs list e.g. "2600 0D00" or "FTA"

# Video / Audio / Resolution
# xres              → Video width (e.g. "1920")
# yres              → Video height + scan type (e.g. "1080i")
# vsize             → WidthxHeight|FPS|Gamma e.g. "1920x1080i|25|SDR"
# Resolution        → WidthxHeight + FPS e.g. "1920x1080i25"
# VideoWidth        → Width only e.g. "1920"
# VideoHeight       → Height only e.g. "1080"
# fps / Framerate   → Frame rate (raw value) e.g. "25000" (represents 25.000)
# tbps              → Transfer rate e.g. "512 kB/s"
# vtype             → Video codec e.g. "HEVC"
# atype             → Current audio track description e.g. "AAC  (eng)"
# avtype            → Combined Video/Audio e.g. "HEVC/AAC"
# AudioCodec        → Audio codec + language e.g. "AAC  eng"
# VideoCodec        → Video codec only e.g. "HEVC"
# AudioDetails      → Current audio track detail e.g. "AAC (eng)"

# Advanced Video / HDR
# gamma / HDRINFO   → Gamma/HDR info e.g. "HDR10"
# HDRType           → HDR type short e.g. "HDR10"
# MEDIA_INFO        → Full media info e.g. "1920x1080p 25fps | HEVC | AAC (eng) | HDR10"
# VSIZE_INFO        → e.g. "1920x1080p | 25fps | HDR | HEVC | AAC eng"

# Service Type & Flags
# ttype             → Tuner/type string e.g. "DVB-S2" or "IPTV"
# IsSatellite       → Boolean e.g. True  (for DVB-S)
# IsSatelliteS      → Boolean e.g. True  (DVB-S legacy)
# IsSatelliteS2     → Boolean e.g. True  (DVB-S2)
# IsCable           → Boolean e.g. False (for DVB-C)
# IsCableC          → Boolean e.g. False
# IsCableC2         → Boolean e.g. False
# IsTerrestrial     → Boolean e.g. False (for DVB-T)
# IsTerrestrialT    → Boolean e.g. False
# IsTerrestrialT2   → Boolean e.g. False
# IsStreamTV        → Boolean e.g. False (True if streaming)
# IsIPTV            → "Yes" / "No" string e.g. "Yes" (for streams)

# Crypto / Free-to-air
# IsCrypted         → Boolean e.g. True
# IsDecrypted       → Boolean e.g. False
# IsFta             → Boolean e.g. False
# EncryptionType    → Encryption system e.g. "Biss" / "PowerVU" / "Viaccess" / "FTA"

# Other booleans & availability
# IsWidescreen      → Boolean e.g. True  (if aspect in WIDESCREEN)
# HasTelext         → Boolean e.g. True  (teletext available)
# HasHBBTV          → Boolean e.g. True  (HbbTV url exists)
# SubservicesAvailable → Boolean e.g. False
# AudioTracksAvailable → Boolean e.g. True
# SubtitlesAvailable → Boolean e.g. True
# SubtitleInfo      → e.g. "3 available" or "None"
# Editmode          → Boolean e.g. False

# Network & Stream info
# IPLOCAL           → Local box IP e.g. "IP : 192.168.1.10"
# StreamFormat      → Stream protocol e.g. "HLS" / "RTSP" / "MPEG-DASH" / "Unknown"
# BufferStatus      → Buffer fill e.g. "85%"
# Latency           → Measured latency e.g. "123.45 ms" or "Timeout"
# ProviderName      → Provider name e.g. "BBC"
# SIGNAL_DB         → Signal SNR e.g. "12.3 dB"

# Volume
# IsVolume          → Formatted volume e.g. "Vol: 50"
# IsVolumeData      → Raw volume e.g. "50"

# Misc / format usage
# format (self.sfmt)→ Custom format string: you can use tokens like %apid %vpid %xres etc.  
#                    Example format usage returns something like: "BBC One HD - 11938 MHz - 27500"

# Examples of combined outputs used by code:
# MEDIA_INFO example: "1920x1080p 25fps | HEVC | AAC (eng) | HDR10"
# VSIZE_INFO example:  "1920x1080p | 25fps | HDR | HEVC | AAC eng"

# ==============================================
# Notes:
# - PIDs/IDs are typically printed as 4-digit HEX (via "%0.4X") in this converter.
# - Some values are only available for DVB services (frequency, symbol rate, transponder data).
#   For IPTV these fields are usually "IPTV"/"Streaming" or skipped (N/A).
# - Exact labels / codec names depend on your image's codec_data/gamma_data maps.
# - You can paste these comment lines into the top of your furyServiceInfoEX file for quick reference.
# ==============================================
...
class furyServiceInfoEX(Poll, Converter, object):
	apid = 0
	vpid = 1
	sid = 2
	onid = 3
