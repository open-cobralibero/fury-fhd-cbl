...
class furyBackdropXDownloadThread(threading.Thread):
    ...
    def search_tmdb(self, dwn_backdrop, title, shortdesc, fulldesc, channel=None, alias_paths=None):
        try:
        ...
        if 'results' in data_json:
            for each in data_json['results']:
                ...
                if media_type in ['serie', 'movie']:
                    ...
                    if bp:
                        backdrop = "https://image.tmdb.org/t/p/w780" + bp
    ...
    def savebackdrop(self, url, callback, alias_paths=None):
        ...
        try:
            ...
            response = getter(url, headers=headers, timeout=(5, 12), stream=True, verify=False)
            response.raise_for_status()
            ...
            if not header_says_jpeg:
                ...
                try:
                    ...
                    response2 = getter(url, headers=headers, timeout=(5, 12), stream=False, verify=False)
