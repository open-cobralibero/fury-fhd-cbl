...
def _clean_title_common(s):
    ...
    try:
        ...
        s = re.sub(r"\bEPISODE\s*\d{1,4}\b", " ", s, flags=re.IGNORECASE)
...
def translate_to_english_query(title):
    ...
    try:
        ...
        if not src:
            return None
        if not _needs_english_translation(src):
            return None
...
def choose_fast_search_methods(channel=None, title=None, shortdesc=None, fulldesc=None):
    try:
        arabic_channel = _is_arabic_channel(channel)
        arabic_event = _is_likely_arabic_event(title, shortdesc, fulldesc)
        if arabic_channel and arabic_event:
            return ('search_tmdb', 'search_google', 'search_tvdb')
        return ('search_tmdb', 'search_tvdb')
    except Exception:
        return ('search_tmdb', 'search_tvdb')
...
def build_title_candidates(title, shortdesc=None, fulldesc=None, channel=None):
    ...
    try:
        ...
        # 1) Raw title first (preserves exact matching when provider already supports locale)
        _add(raw)
        ...
        # 3) Early English translation fallback (so fast search tries it within first slots)
        translated_main = translate_to_english_query(cleaned or raw)
        if translated_main:
            _add(translated_main)
            _add(_clean_title_common(translated_main))

        # 4) Arabic normalization (helps loose matching on some sources)
        if _contains_arabic(cleaned):
            _add(_normalize_arabic(cleaned))

        # 5) Some EPGs include a better subtitle after dash or colon; keep it early
        tail = None
        if raw and ("-" in raw or ":" in raw):
            tail = _clean_title_common(re.split(r"[-:]", raw)[-1].strip())
            _add(tail)
            translated_tail = translate_to_english_query(tail)
            if translated_tail:
                _add(translated_tail)

        # 6) English fragments from title/desc (often official/original title is present there)
        latin = _extract_latin_candidates(raw, shortdesc, fulldesc)
        for x in latin:
            _add(x)

        # 7) Disambiguate with channel name for Google-like sources (kept later)
        if channel:
            try:
                ch = _to_text(channel)
                ch = re.sub(r"\s+", " ", ch).strip()
        ...
        while True:
            ...
            for search_method in search_methods:
                ...
                if not cand_list:
                    continue
                max_try = 3
                if search_method.__name__ in ("search_google", "search_programmetv_google", "search_molotov_google"):
                    max_try = 3 if _is_likely_arabic_event(canal[5], canal[4], canal[3]) else 1
    ...
    def run(self):
        ...
        while True:
            ...
            for service in services:
                try:
                    events = epgcache.lookupEvent(['IBDCTESX', (service, 0, -1, 4320)])  # 3 days lookahead
                    newfd = 0
                    newcn = None
    ...
    def changed(self, what):
        ...
        try:
            ...
            if existing:
                ...
                self._debounceTimer.start(80, True)
