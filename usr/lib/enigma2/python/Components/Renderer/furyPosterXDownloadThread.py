...
from .furyConverlibr import quoteEventName
...
class furyPosterXDownloadThread(threading.Thread):
    ...
    def search_tmdb(self, dwn_poster, title, shortdesc, fulldesc, channel=None):
        """Search TMDB first (fast). Return (ok, log). Must NOT claim success unless a file is written."""
        try:
            ...
            for lang_try in lang_candidates:
                ...
                try:
                    response = self.http.get(url, params=params, headers=headers, timeout=(3, 8), verify=False)
                except Exception:
                    response = requests.get(url, params=params, headers=headers, timeout=(3, 8), verify=False)
    ...
    def downloadData2(self, data):
        """Select the best TMDB result that has a valid poster_path and save it to self.dwn_poster."""
        try:
            ...
            for each in results:
                try:
                    ...
                    poster_url = "https://image.tmdb.org/t/p/w342{}".format(poster_path)
    ...
    def savePoster(self, url, callback):
        ...
        try:
            ...
            response = get(url, headers=headers, stream=True, timeout=(2, 6))
            response.raise_for_status()
            ...
            # --- PATCH: normalize downloaded image to real JPEG (fast path for JPEG)
            try:
                if os.path.exists(callback) and os.path.getsize(callback) > 0:
                    # If already JPEG, keep as-is (fast, low CPU)
                    with open(callback, "rb") as f:
                        head = f.read(3)
                    if head != b"\xff\xd8\xff":
                        img = Image.open(callback)
                        img = img.convert("RGB")
                        # Baseline JPEG for enigma2 loadJPG compatibility
                        img.save(callback, "JPEG", quality=88)
                        img.close()
            except Exception:
                # If PIL cannot decode or write fails, drop the file
                try:
                    os.remove(callback)
                except Exception:
                    pass
