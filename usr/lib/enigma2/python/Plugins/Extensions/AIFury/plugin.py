...
PluginLanguageDomain = "AIFury"
PluginLanguagePath = "Extensions/AIFury/locale"

gettext.bindtextdomain(PluginLanguageDomain, resolveFilename(SCOPE_PLUGINS, PluginLanguagePath))
# Force gettext to treat translations as UTF-8 (important on some Python2-based images)
try:
...
def _(txt):
    ...
    # 1) Ensure we return unicode (Py2) / str (Py3)
    try:
    ...
    # 2) Mojibake repair: if UTF-8 bytes were decoded as latin-1 somewhere,
    # you'll see strings like "Ø§Ù„..." instead of Arabic. Convert back.
    try:
        if isinstance(t, str):
            if ("Ø" in t) or ("Ù" in t) or ("Ã" in t):
...
_RE_MULTI_SPACE = re.compile(u"[ \t\xa0]+", re.UNICODE)
_RE_TOKEN_PROTECT = re.compile(
    u"(https?://\\S+|www\\.\\S+|\\b[\\w.+-]+@[\\w.-]+\\.[A-Za-z]{2,}\\b|\\b\\d{1,2}:\\d{2}(?:\\s?[APMapm]{2})?\\b|\\bS\\d{1,2}E\\d{1,2}\\b|\\bE\\d{1,3}\\b|\\b\\d{3,4}p\\b|\\b[A-Z0-9][A-Z0-9_./-]{1,11}\\b)",
    re.UNICODE,
)
_RE_ARABIC_SPAN = re.compile(u"[\u0600-\u06FF]+(?:[\u0600-\u06FFـ-]+)*", re.UNICODE)
_RE_ARABIC_WORD = re.compile(u"[\u0600-\u06FF]", re.UNICODE)
_RE_LATIN_WORD = re.compile(u"[A-Za-z]{3,}", re.UNICODE)
_RE_SPLIT_NEWLINES = re.compile(u"(\\n+)", re.UNICODE)
_RE_SPLIT_SENTENCES = re.compile(u"([.!?؟؛;]+\\s+)", re.UNICODE)
...
def _repair_mojibake_text(text):
    ...
    if not t:
        return t
    try:
        if (u"Ø" in t) or (u"Ù" in t) or (u"Ã" in t):
...
def _strip_invisible_marks(text):
    t = _to_unicode(text)
    if not t:
        return t
    for ch in (u"\ufeff", u"\u200e", u"\u200f", u"\u202a", u"\u202b", u"\u202c", u"\u202d", u"\u202e", u"\u2066", u"\u2067", u"\u2068", u"\u2069"):
        t = t.replace(ch, u"")
    return t


def _normalize_translation_text(text):
    t = _strip_invisible_marks(_repair_mojibake_text(text))
    if not t:
        return u""
    t = t.replace(u"\r\n", u"\n").replace(u"\r", u"\n").replace(u"\t", u" ")
    lines = []
    for line in t.split(u"\n"):
        line = _RE_MULTI_SPACE.sub(u" ", line).strip()
        lines.append(line)
    t = u"\n".join(lines)
    while u"\n\n\n" in t:
        t = t.replace(u"\n\n\n", u"\n\n")
...
def _protect_translation_tokens(text, target_lang=None):
    t = _to_unicode(text)
    tokens = []

    def _repl(match):
        token = _to_unicode(match.group(0))
        key = u"AIFPH%04dX" % len(tokens)
        tokens.append((key, token))
        return key

    try:
        t = _RE_TOKEN_PROTECT.sub(_repl, t)
    except Exception:
        pass

    try:
        tl = (target_lang or u"").lower()
    except Exception:
        tl = u""

    if tl.startswith(u"ar"):
        try:
            t = _RE_ARABIC_SPAN.sub(_repl, t)
        except Exception:
            pass
...
def _restore_translation_tokens(text, tokens):
    t = _to_unicode(text)
    for key, token in (tokens or []):
        try:
...
def _split_translation_chunks(text, max_chars=700):
    t = _to_unicode(text)
    if not t:
        return []
    try:
    ...
    if len(t) <= max_chars and t.count(u"\n") < 3:
        return [t]

    parts = []
    for block in _RE_SPLIT_NEWLINES.split(t):
        if not block:
            continue
        ...
        sentence_parts = _RE_SPLIT_SENTENCES.split(block)
        current = u""
        for piece in sentence_parts:
            ...
            if len(piece) > max_chars:
                ...
                for word in words:
                    if not word:
                        continue
                    item = word if not buf else (buf + u" " + word)
                    if len(item) <= max_chars:
                        ...
                        buf = word
                if buf:
                    if current and len(current) + len(buf) + 1 <= max_chars:
                        current = current + u" " + buf
                    else:
                        if current:
...
def _contains_arabic(text):
    try:
        return bool(_RE_ARABIC_WORD.search(_to_unicode(text)))
    except Exception:
        return False


def _looks_untranslated_for_arabic_result(original, translated):
    o = _normalize_translation_text(original)
    t = _normalize_translation_text(translated)
    if not o or not t:
        return False
    if o != t:
        return False
    if is_mostly_arabic(o):
        return False
    try:
        latin_words = _RE_LATIN_WORD.findall(o)
    except Exception:
        latin_words = []
...
def _cleanup_spacing_and_punctuation(text):
    t = _to_unicode(text)
    if not t:
        return t
    while u"  " in t:
        t = t.replace(u"  ", u" ")
    try:
        t = re.sub(u"\\s+([،؛:!?؟,;\\.])", u"\\1", t, flags=re.UNICODE)
    except Exception:
        pass
    try:
        t = re.sub(u"([\\(\\[\\{])\\s+", u"\\1", t, flags=re.UNICODE)
...
def post_process_arabic_text(original, translated):
    ...
    - تنظيف مسافات زائدة وعلامات الترقيم العربية.
    """
    o = _normalize_translation_text(original)
    t = _normalize_translation_text(translated)
    ...
    t = _cleanup_spacing_and_punctuation(t)

    # تحويل الترقيم الإنجليزي لعربي عند وجود سياق عربي
    try:
        t = re.sub(u"(?<=[\u0600-\u06FF]),", u"،", t, flags=re.UNICODE)
        t = re.sub(u",(?=[\u0600-\u06FF])", u"،", t, flags=re.UNICODE)
        t = re.sub(u"(?<=[\u0600-\u06FF]);", u"؛", t, flags=re.UNICODE)
        t = re.sub(u";(?=[\u0600-\u06FF])", u"؛", t, flags=re.UNICODE)
        t = re.sub(u"(?<=[\u0600-\u06FF])\\?", u"؟", t, flags=re.UNICODE)
        t = re.sub(u"\\?(?=[\u0600-\u06FF])", u"؟", t, flags=re.UNICODE)
        t = re.sub(u"([،؛:!?؟\\.])([\u0600-\u06FFA-Za-z0-9])", u"\\1 \\2", t, flags=re.UNICODE)
    except Exception:
        pass
    ...
    return _cleanup_spacing_and_punctuation(u" ".join(new_t_words).strip())


def _translate_google_chunk(text, target_lang=TARGET_LANG, timeout=DEFAULT_TIMEOUT, log_path=None, log_fn=None,
                            retries=0, min_interval_ms=None, backoff_base_s=0.35, backoff_jitter_s=0.2):
    chunk = _to_unicode(text)
    if not chunk:
        return u""
    ...
    params = {
        ...
        "ie": "UTF-8",
        "oe": "UTF-8",
        "q": chunk.encode("utf-8") if not PY3 else chunk,
    }
    ...
    for attempt in range(max_attempts):
        try:
            ...
            if isinstance(data, list) and data:
                for item in data[0] or []:
                    if item and isinstance(item, list) and item[0]:
                        translated_chunks.append(_to_unicode(item[0]))
            ...
            try:
                ...
                sample = (chunk[:80] + u"...") if len(chunk) > 80 else chunk
...
def translate_text(text, target_lang=TARGET_LANG, timeout=DEFAULT_TIMEOUT, log_path=None, log_fn=None,
    ...
    - Normalize input, protect sensitive tokens (times/codes/URLs/Arabic words), and chunk long texts.
    ...
    if _tl.startswith(u"ar") and is_mostly_arabic(text):
        return text

    prepared, protected_tokens = _protect_translation_tokens(text, _tl)
    chunks = _split_translation_chunks(prepared, max_chars=700)
    if not chunks:
        return u""
    ...
    translated_text = u"".join(translated_parts).strip()
    translated_text = _restore_translation_tokens(translated_text, protected_tokens)
    translated_text = _repair_mojibake_text(translated_text)

    if _tl.startswith(u"ar"):
        if _looks_untranslated_for_arabic_result(text, translated_text):
            return u""
        translated_text = post_process_arabic_text(text, translated_text)
    else:
        translated_text = _cleanup_spacing_and_punctuation(translated_text)
...
def get_default_epg_cache_path(base_dir=None):
    """Return the default persistent EPG cache path and keep backward compatibility with the legacy filename."""
    try:
        base_dir = (base_dir or "").strip()
    except Exception:
        base_dir = ""
    if not base_dir:
        base_dir = os.path.join("/tmp", "AIFury")

    new_path = os.path.join(base_dir, "aifury_epg_cache.json")
    legacy_path = os.path.join(base_dir, "aifury_epg_on_cache")
    try:
...
config.plugins.aifury.epg_title_lang = ConfigSelection(
    default="",
    choices=[("", _("Disabled"))] + AIFury_LANG_CHOICES,
)
...
# Translate InfoBar (EpgFuryEvent converter) independently from EPG cache translation
config.plugins.aifury.infobar_translate = ConfigYesNo(default=True)
...
class _AIFuryWorkerPool(object):
    def __init__(self, workers=4, logger=None):
        if _queue is None:
            raise RuntimeError('queue module is not available')
        self._logger = logger
        ...
        for idx in range(workers):
            t2 = threading.Thread(target=self._worker, name='AIFuryWorker-%d' % idx)
            t2.setDaemon(True)
            t2.start()
    ...
    def submit(self, fn):
        ...
        try:
            ...
            self._log('worker submit error: %s' % e)
            return None
...
class AIFuryController(object):
    ...
    def __init__(self, session):
        ...
        if not cfg2_path:
            ...
            cfg2_path = get_default_epg_cache_path(base_dir)
        ...
        self._cache_last_save_ts = time.time()
        self._cache_save_min_gap_s = 3.0
    ...
    def _make_text_cache_key(self, lang, text):
        try:
            return "%s|%s" % (lang or "", _normalize_translation_text(text))
        except Exception:
            return "%s|%s" % (lang or "", _to_unicode(text))

    def _cache_lookup_value(self, cache_key):
        try:
            with self._cache_lock:
    ...
    def _save_cache_debounced(self, force=False):
        try:
            now = time.time()
            dirty = int(getattr(self, "_cache_dirty", 0) or 0)
            interval = int(getattr(self, "_cache_save_interval", 0) or 0)
            min_gap = float(getattr(self, "_cache_save_min_gap_s", 0.0) or 0.0)
            last = float(getattr(self, "_cache_last_save_ts", 0.0) or 0.0)
            should_save = False
            if force:
                should_save = True
            elif dirty > 0:
                if interval and dirty >= interval:
                    should_save = True
                elif min_gap > 0 and (now - last) >= min_gap:
                    should_save = True
            if should_save:
                self._save_cache(force=bool(force))
                self._cache_last_save_ts = time.time()
        except Exception:
            pass

    def _remember_translation(self, cache_key, value):
        if not cache_key:
            return
        try:
            with self._cache_lock:
        ...
        try:
            self._cache_dirty = int(getattr(self, "_cache_dirty", 0) or 0) + 1
        except Exception:
            self._cache_dirty = 1
    ...
    def _mark_translation_failure(self, cache_key):
        if not cache_key:
            return
        try:
            now = time.time()
            with self._pending_lock:
                self._failed_cache[cache_key] = now
                if len(self._failed_cache) > 4096:
                    for k in list(self._failed_cache.keys())[:1024]:
                        try:
                            if (now - float(self._failed_cache.get(k) or 0)) > 300:
                                del self._failed_cache[k]
                        except Exception:
                            pass
        except Exception:
            pass

    def _is_recent_translation_failure(self, cache_key):
        if not cache_key:
            return False
        try:
            cooldown = float(getattr(self, "_failed_cooldown_s", 20.0) or 20.0)
        except Exception:
            cooldown = 20.0
        try:
            with self._pending_lock:
                ts = self._failed_cache.get(cache_key)
                if not ts:
                    return False
                if (time.time() - float(ts)) < cooldown:
                    return True
                try:
                    del self._failed_cache[cache_key]
                except Exception:
    ...
    def _begin_pending_translation(self, cache_key, callback=None):
        with self._pending_lock:
            if cache_key in self._pending:
                if callback is not None:
                    self._pending_waiters.setdefault(cache_key, []).append(callback)
                return False
            self._pending.add(cache_key)
            self._pending_waiters[cache_key] = [callback] if callback is not None else []
            return True

    def _finish_pending_translation(self, cache_key, result=None):
        callbacks = []
        try:
            with self._pending_lock:
                try:
                    ...
                    pass
                callbacks = list(self._pending_waiters.pop(cache_key, []) or [])
        except Exception:
            callbacks = []

        if result is None:
            result = self._cache_lookup_value(cache_key)

        for cb in callbacks:
            try:
                self._call_ui(lambda callback=cb, value=result: callback(value))
            except Exception:
                pass
    ...
    def translate_to_cached_or_async(self, text, lang):
        ...
        if not text or not self.enabled or not lang:
            return text
        cache_key = self._make_text_cache_key(lang, text)
        cached = self._cache_lookup_value(cache_key)
        if cached is not None:
            return cached
        if self._is_recent_translation_failure(cache_key):
            return text
        if not self._begin_pending_translation(cache_key):
            return text
        ...
        def worker():
            ...
            try:
                ...
                self._finish_pending_translation(cache_key, result)
    ...
    def translate_to_async_with_callback(self, text, lang, callback):
        ...
        cached = self._cache_lookup_value(cache_key)
        if cached is not None:
            self._call_ui(lambda: callback(cached))
            return
        if self._is_recent_translation_failure(cache_key):
            self._call_ui(lambda: callback(text))
            return
        if not self._begin_pending_translation(cache_key, callback=callback):
            return
    ...
    def _translate_common(self, text, lang):
        """Shared translation path with cache, cooldown, retries, and persistence."""
        if not text:
            return text
        if not lang:
            return text

        source = _normalize_translation_text(text)
        if not source:
            return text

        cache_key = self._make_text_cache_key(lang, source)
        cached = self._cache_lookup_value(cache_key)
        if cached is not None:
            return cached
        ...
        for attempt in range(max(0, retries) + 1):
            try:
                ...
                out = translator(source, lang, timeout=timeout_s)
                out = _to_unicode(out)
                if not out or not out.strip():
                    raise ValueError("translation returned empty")
                out = out.strip()
                self._remember_translation(cache_key, out)
                try:
                    dt = time.time() - t0
                    sample = (source[:80] + u"...") if len(source) > 80 else source
                    self._log("translated (%.3fs) lang=%s text=%s" % (dt, lang, sample))
                except Exception:
                    pass
                return out
            except Exception as e:
        ...
        if last_err is not None:
            try:
                self._log("translate failed lang=%s: %s" % (lang, last_err))
            except Exception:
                pass
        self._mark_translation_failure(cache_key)
        return text
    ...
    def translate_to(self, text, lang):
        ...
        """
        return self._translate_common(text, lang)
    ...
    def translate_async(self, text):
        ...
        lang = config.plugins.aifury.language.value
        cache_key = self._make_text_cache_key(lang, text)
        if self._cache_lookup_value(cache_key) is not None:
            return
        if self._is_recent_translation_failure(cache_key):
            return
        if not self._begin_pending_translation(cache_key):
                ...
                try:
                    ...
                    try:
                        ...
                        if ctrl is not None and ctrl.enabled:
                            ...
                            key_name = ctrl._make_text_cache_key(title_lang, name or "")
                            key_desc = ctrl._make_text_cache_key(descr_lang, description or "")
...
class AIFuryInfoScreen(Screen):
    ...
    def __init__(self, session):
        ...
        if not (title_lang or descr_lang):
            txt.append(_("EPG translation language: Disabled"))
        else:
            txt.append(_("EPG title language : %s") % (title_lang or ""))
            txt.append(_("EPG descr language : %s") % (descr_lang or ""))
            if ctrl:
                try:
                    ...
                    total = 0
                txt.append("")
                txt.append(_("Translated events in cache: %d") % int(total))
            else:
                txt.append("")
                txt.append(_("Translated events in cache: (controller not ready)"))
        ...
        if bref:
            try:
                ...
                bname = ""
            txt.append("")
            txt.append(_("Bouquet (package): %s") % (bname or "Unknown"))
            if ctrl and (title_lang or descr_lang):
                ...
                try:
                    ...
                    bcount = 0
                txt.append(_("Translated events in bouquet: %d") % int(bcount))
                txt.append(_("Bouquet services: %d") % (len(srefs) if srefs else 0))
        else:
            txt.append("")
            txt.append(_("Bouquet (package): Not selected yet"))
        ...
        txt.append("")
        txt.append(_("Auto translate EPG in background: %s") % (_("Enabled") if bg_enabled else _("Disabled")))

        if bg_enabled and ctrl:
            st = ctrl.get_bg_stats()
            try:
                txt.append(_("Background runs: %d") % int(st.get("runs", 0) or 0))
            except Exception:
                pass
            try:
                txt.append(_("Translated in background: %d") % int(st.get("translated", 0) or 0))
            except Exception:
                pass
            try:
                txt.append(_("Imported in background: %d") % int(st.get("imported", 0) or 0))
            except Exception:
                pass
            try:
            ...
            if lts:
                try:
                    ...
                    when = str(lts)
                txt.append(_("Last background run: %s") % when)
            try:
                lsrv = str(st.get("last_service", "") or "")
            except Exception:
                lsrv = ""
            if lsrv:
                txt.append(_("Last service: %s") % lsrv)
...
class AIFuryLanguageScreen(Screen):
    ...
    def __init__(self, session, current_value=None, include_disabled=False):
        ...
        try:
            self.setTitle(_("Select translation language"))
        except Exception:
            pass
        self.session = session
        ...
        if include_disabled:
            self.lang_choices = [("", _("Disabled"))] + self.lang_choices
...
class AIFurySetup(Screen, ConfigListScreen):
    skin = (
        ...
        '<widget name="key_help" position="20,370" size="760,20" font="Regular;18" halign="center" />'
        '</screen>'
    )
    ...
    def __init__(self, session):
        ...
        try:
            self.setTitle(_("AIFury Setup"))
        except Exception:
            pass
        self.session = session

        self["info"] = Label(_("AIFury - translation control"))
        self["key_help"] = Label(_("OK = Choose option   |   Green = Save   |   Yellow = Clear cache   |   Blue = Stats   |   Red/EXIT = Cancel"))
    ...
    def _getConfigList(self):
        ...
        lst.append(getConfigListEntry(_("Enable Translate"), config.plugins.aifury.enabled))

        if not config.plugins.aifury.enabled.value:
            return lst

        lst.append(getConfigListEntry(_("Translation language"), config.plugins.aifury.language))
        lst.append(getConfigListEntry(_("Enable EPG Translation"), config.plugins.aifury.enable_translate_current_event))

        
        # InfoBar translation (EpgFuryEvent) can be controlled separately
        if hasattr(config.plugins.aifury, "infobar_translate"):
            lst.append(getConfigListEntry(_("Translate InfoBar"), config.plugins.aifury.infobar_translate))
        ...
        if config.plugins.aifury.enable_translate_current_event.value:
            ...
            lst.append(getConfigListEntry(_("Translate Current Event"), self._cfg_translate_current_epg))
            try:
            ...
            lst.append(getConfigListEntry(_("Translate bouquet (choose from list)"), self._cfg_translate_choose_bouquet))
            ...
            lst.append(getConfigListEntry(_("Translate bouquet in background"), self._cfg_translate_choose_bouquet_bg))
            lst.append(getConfigListEntry(_("Notify translation end"), config.plugins.aifury.bouquet_bg_notify_done))
            lst.append(getConfigListEntry(_("Done notification timeout"), config.plugins.aifury.done_notify_timeout))
            lst.append(getConfigListEntry(_("EPG title language"), config.plugins.aifury.epg_title_lang))
            lst.append(getConfigListEntry(_("Auto restore translated EPG on zap"), config.plugins.aifury.auto_restore_epg))

        if config.plugins.aifury.auto_restore_epg.value:
            lst.append(getConfigListEntry(_("Periodic restore interval"), config.plugins.aifury.keep_restore_interval))
            lst.append(getConfigListEntry(_("Auto translate EPG in background"), config.plugins.aifury.auto_translate_epg))

        lst.append(getConfigListEntry(_("Auto translate horizon"), config.plugins.aifury.auto_translate_horizon))
        lst.append(getConfigListEntry(_("Auto translate min gap (minutes)"), config.plugins.aifury.auto_translate_min_gap))
        lst.append(getConfigListEntry(_("Auto translate max events"), config.plugins.aifury.auto_translate_max_events))
        lst.append(getConfigListEntry(_("Main path"), config.plugins.aifury.cachepath))
        lst.append(getConfigListEntry(_("EPG path"), config.plugins.aifury.epgcachepath))
        lst.append(getConfigListEntry(_("Network timeout (sec)"), config.plugins.aifury.req_timeout))
       # lst.append(getConfigListEntry(_("Retry count"), config.plugins.aifury.req_retries))
        lst.append(getConfigListEntry(_("Min request interval (ms)"), config.plugins.aifury.min_interval_ms))
        lst.append(getConfigListEntry(_("Worker threads"), config.plugins.aifury.workers))

        lst.append(getConfigListEntry(_("Reset: Defaults + Caches"), config.plugins.aifury.maint_reset_all))
        lst.append(getConfigListEntry(_("Reset: Defaults only"), config.plugins.aifury.maint_reset_defaults))
        lst.append(getConfigListEntry(_("Clear caches only"), config.plugins.aifury.maint_clear_caches))
        return lst
    ...
    def _restore_original_epg(self):
        ...
        def _run():
            try:
                ...
                if ref is not None:
                    # Fast + predictable: revert only the currently-played service if known
                    try:
                        ctrl.revert_translated_epg_to_original(ref, keep_cache=True)
                    except Exception:
                        # Fallback to reverting everything if per-service revert fails
    ...
    def _translateCurrentEventToggled(self, *args, **kwargs):
        ...
        try:
            if not config.plugins.aifury.enable_translate_current_event.value:
                ...
                self._pending_restore_original = False
                self._restore_service_ref = None
                # EPG translation enabled again: read previously saved translations from the
                # configured save path, then force a fresh translation pass for the current service.
                try:
                    ctrl = AIFuryController.instance
                    if ctrl is not None:
                        try:
                        ...
                        if sref is not None:
                            try:
                                ctrl.schedule_epg_restore(sref, delay_ms=0)
                            except Exception:
                                pass
                            try:
                                ctrl.schedule_auto_translate(sref, delay_ms=500, force=True)
                            except Exception:
                                pass
                except Exception:
                    pass
    ...
    def _translateEnabledToggled(self, *args, **kwargs):
        ...
        try:
            if not config.plugins.aifury.enabled.value:
                ...
                self._pending_restore_original = False
                self._restore_service_ref = None
                # Plugin enabled again: restore saved translations from disk cache (if EPG translation is enabled)
                try:
                    if config.plugins.aifury.enable_translate_current_event.value:
                        ctrl = AIFuryController.instance
                        if ctrl is not None:
        ...
        try:
            if hasattr(self, "_cfg_translate_current_epg") and cfg is self._cfg_translate_current_epg:
                if (not hasattr(config.plugins.aifury, "enable_translate_current_event")) or (not config.plugins.aifury.enable_translate_current_event.value):
                    try:
                        self.session.open(MessageBox, _("EPG translation is disabled in settings."), MessageBox.TYPE_INFO, timeout=4)
                    except Exception:
                        pass
                    return
...
# ---------- 2) ConfigSelection / YesNo أخرى ----------
    ...
    def _start_translate_current_epg(self):
        ...
        if ctrl is None:
            try:
                self.session.open(MessageBox, _("AIFury controller is not initialized."), MessageBox.TYPE_ERROR, timeout=5)
            except Exception:
                pass
            return
        ...
        if not (title_lang or descr_lang):
            try:
                self.session.open(MessageBox, _("Select EPG title/description language first."), MessageBox.TYPE_INFO, timeout=5)
            except Exception:
                pass
            return
        ...
        if sref is None:
            try:
                self.session.open(MessageBox, _("No current service is available."), MessageBox.TYPE_ERROR, timeout=5)
            except Exception:
                pass
            return
        ...
        try:
            ...
            try:
                self.session.open(MessageBox, _("Cannot start EPG translation: %s") % e, MessageBox.TYPE_ERROR, timeout=6)
            except Exception:
                pass
    ...
    def _openBouquetChooser(self):
        ...
        if not bouquets:
            try:
                self.session.open(MessageBox, _("No bouquets found."), MessageBox.TYPE_INFO, timeout=4)
            except Exception:
                pass
            return
    ...
    def _start_translate_bouquet(self, bouq_ref):
        ...
        if not bouq_ref:
            try:
                self.session.open(MessageBox, _("Bouquet is empty/invalid."), MessageBox.TYPE_INFO, timeout=4)
            except Exception:
                pass
            return
        ...
        if not services:
            try:
                ...
                self.session.open(MessageBox, _("No playable services found in: %s") % bn, MessageBox.TYPE_INFO, timeout=5)
            except Exception:
                pass
            return
    ...
    def _start_translate_favourites_bouquet(self):
        ...
        if not services:
            try:
                self.session.open(MessageBox, _("No services found in Favourites bouquet."), MessageBox.TYPE_INFO, timeout=6)
            except Exception:
                pass
            return
        ...
        if ctrl is not None:
            ...
            try:
                ...
                if not epg_path:
                    ...
                    epg_path = get_default_epg_cache_path(base_dir)
    ...
    def keyClear(self):
        ...
        if ctrl is None:
            self.session.open(
                MessageBox,
                _("Controller not ready, cannot clear cache."),
